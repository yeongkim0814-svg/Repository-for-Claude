...
- `fit_internals.py` **작성 및 실행 완료** — 결과를 실제로 만들어보고 시각 확인함:
  ...
  - 뒷판 v1(트레이 + 체결 보스 + 서보 축 받침 자리) 생성 — **아직 임시 버전**:
    화면·버튼 클러스터 영역만 커버(다리 쪽 마운트 2곳은 다음 버전에서),
    서보 축 받침 높이는 잠정 추정값(TODO로 코드에 표시됨) — 실물/사진 대조 필요.
    내부 스탠드오프(OLED/보드/서보 실제 고정부)는 아직 없음, 다음 단계.
