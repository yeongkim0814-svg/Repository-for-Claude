...
64×32 캔버스(128×64 흑백 OLED에 정확히 2배로 꽉 찬다)를 팔레트 인덱스(0~3)로
만든 뒤 RLE + base64 로 감싼다.
...
| 0 | 꺼짐 | 배경 |
| 1 | 켜짐 | 이목구비 |
| 2 | 켜짐 | 볼 (기기에서는 점무늬로 그려 구분) |
| 3 | 켜짐 | 하이라이트 |

흑백 OLED라 실제로 빛나는 건 켜짐/꺼짐 2단계뿐이지만, 인덱스 4개(2비트)는 그대로 쓴다.
그래야 코드를 더 안 건드리고, 색이 있는 화면으로 나중에 바꾸더라도 프로토콜이 안 바뀐다.

실측 크기 (`./tools/test_codec.sh`):
...
BMO 안에는 **한글 폰트가 없다.** 웹앱이 브라우저 폰트로 글씨를 64×32 캔버스에 그린 뒤
